...
const envVarSchema = joi.object().keys({
  ...
  SECRET_JSONWEBTOKEN: joi
    .string()
    .required()
    .description("Secret jsonwebtoken"),
  APP_CLIENT_ID: joi.string().required().description("Google client id"),
  APP_CLIENT_SECRET: joi
    .string()
    .required()
    .description("Google app client secret"),
  APP_REFRESH_TOKEN: joi
    .string()
    .required()
    .description("Google app client refresh token"),
  APP_USER_EMAIL: joi.string().required().description("Email"),
  APP_USER_EMAIL_PW: joi.string().required().description("Email pw"),
  FE_URL: joi.string().required().description("Frontend URL"),
  APP_GEOCODE_APIKEY: joi.string().required().description("Frontend URL"),
});
...
export const config = {
  ...
  APP_USER_EMAIL_PW: envVars.APP_USER_EMAIL_PW,
  APP_GEOCODE_APIKEY: envVars.APP_GEOCODE_APIKEY,
  mongodb: {
